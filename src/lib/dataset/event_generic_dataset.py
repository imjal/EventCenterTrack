...
class EventGenericDataset(data.IterableDataset):
  ...
  def __init__(self, opt=None, split=None, ann_path=None):
    super(EventGenericDataset, self).__init__()
    ...
    if opt.tracking:
      ...
      #   self.fake_video_data() # TODO: look @ this
  ...
  def __next__(self):
    opt = self.opt
    img, anns, img_info, img_path = self._load_data()
    # TODO: change this definition in order to load event camera data, with proper annotations as well
    ...
    if opt.tracking: # get the previous image for tracking output
      # pre_image, pre_anns, frame_dist =
      # self._load_pre_data(
      #   img_info['video_id'], img_info['frame_id'], 
      #   img_info['sensor_id'] if 'sensor_id' in img_info else 1)
      pre_image = self.prev_im
      pre_anns = self.prev_anns
      if flipped:
        pre_image = pre_image[:, ::-1, :].copy()
        pre_anns = self._flip_anns(pre_anns, width)
      if opt.same_aug_pre and frame_dist != 0: # not eq to zero - we don't have stream data
        trans_input_pre = trans_input 
        trans_output_pre = trans_output
      else: # we need to augment the same data
  ...
  def _load_data(self):
    # need to grab the event data
    img, anns, img_info = next(self.stream)
    img_path = ""
  ...
  def _get_input(self, img, trans_input):
    ...
    inp = inp.transpose(2, 0, 1) # make the image the normal way
    return inp
